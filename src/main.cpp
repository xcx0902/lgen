...
inline int selectBot() {
    ...
    printf("Press d to download the bot's source code\n");
    gotoxy(2, 1);
    printf(">>");
    ...
    while (input != 13) {
        ...
        switch (tolower(input)) {
            ...
            case 'v': system(botSourceView[choice]); break;
            case 'd':
                gotoxy(7, 1);
                system(botSourceDownload[choice]);
                break;
        }
        gotoxy(choice + 1, 1);
        printf(">>");
...
int main() {
    ...
    system("title Local Generals v1.5.0");
    initattr();
    hideCursor();
